...
with open( "Y:/Inventory/Road Inventory Processes/Checks/Logs/Domain_logs.txt", "w" ) as logfile:
    logfile.write( "DataReviewerBatch_Domain.py" + '\n\n' + "Start Time:\t" + str( start )[:-7] + '\n\n' )
...
def writeMsg( message ):
    ...
    with open( "Y:/Inventory/Road Inventory Processes/Checks/Logs/Domain_logs.txt", "a") as logfile:
...
reviewer_db = "Y:/Inventory/Road Inventory Processes/Checks/Databases/Domain_Checks.gdb"
rbj_path = "Y:/Inventory/Road Inventory Processes/Checks/RBJs/Domain_Checks/"
rbj = ["Dom1.rbj", "Dom2.rbj", "Dom3.rbj", "Dom4.rbj", "Dom5.rbj", "Dom6.rbj", "Dom7.rbj" ]
# rbj = ["Dom7.rbj" ]
...
saveLayer = reviewer_db + "/Domain_Line_Review" + ("%s_%s" % (now.month, now.day))
# Write logs make a layer and join the tables with the errors to the route events
# For line events:
now = datetime.datetime.now()
